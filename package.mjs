...
try {
  ...
  const sourceStage=mkdtempSync(join(tmpdir(),'review-brief-source-'));try{const sourceFiles=walk(root).filter(f=>!f.startsWith('dist/')&&!f.startsWith('.git/'));const sourcePrefix=`${stem}-source`;copyStable(root,join(sourceStage,sourcePrefix),sourceFiles);const source=join(dist,`${stem}-source.zip`);rmSync(source,{force:true});zip(sourceStage,source,sourceFiles.map(file=>`${sourcePrefix}/${file}`));safeArchive(source);const sums=`${sha(plugin)}  ${basename(plugin)}\n${sha(join(dist,`${stem}-skill.zip`))}  ${stem}-skill.zip\n${sha(source)}  ${basename(source)}\n`;writeFileSync(join(dist,'SHA256SUMS.txt'),sums);}finally{rmSync(sourceStage,{recursive:true,force:true});}
